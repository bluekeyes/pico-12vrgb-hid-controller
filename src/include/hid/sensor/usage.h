...
/**
 * Note: Per Windows documentation, values used as selector report items should
 * be 1-indexed, even though the spec implies they are 0 indexed. It's unclear
 * if this behavior is true for other platforms.
 */

enum SensorState {
    SENSOR_STATE_UNDEFINED      = 0x1,
    SENSOR_STATE_READY          = 0x2,
    SENSOR_STATE_NOT_AVAILABLE  = 0x3,
    SENSOR_STATE_NO_DATA        = 0x4,
    SENSOR_STATE_INITIALIZING   = 0x5,
    SENSOR_STATE_ACCESS_DENIED  = 0x6,
    SENSOR_STATE_ERROR          = 0x7,
};

enum SensorEvent {
    SENSOR_EVENT_UNKNOWN            = 0x1,
    SENSOR_EVENT_STATE_CHANGED      = 0x2,
    SENSOR_EVENT_PROPERTY_CHANGED   = 0x3,
    SENSOR_EVENT_DATA_UPDATED       = 0x4,
    SENSOR_EVENT_POLL_RESPONSE      = 0x5,
    SENSOR_EVENT_CHANGE_SENSITIVITY = 0x6,
};

enum SensorConnectionType {
    SENSOR_CONNECTION_TYPE_PC_INTEGRATED    = 0x1,
    SENSOR_CONNECTION_TYPE_PC_ATTACHED      = 0x2,
    SENSOR_CONNECTION_TYPE_PC_EXTERNAL      = 0x3,
};

enum SensorReportingState {
    SENSOR_REPORTING_STATE_REPORT_NO_EVENTS                 = 0x1,
    SENSOR_REPORTING_STATE_REPORT_ALL_EVENTS                = 0x2,
    SENSOR_REPORTING_STATE_REPORT_THRESHOLD_EVENTS          = 0x3,
    SENSOR_REPORTING_STATE_REPORT_WAKE_ON_NO_EVENTS         = 0x4,
    SENSOR_REPORTING_STATE_REPORT_WAKE_ON_ALL_EVENTS        = 0x5,
    SENSOR_REPORTING_STATE_REPORT_WAKE_ON_THRESHOLD_EVENTS  = 0x6,
};

enum SensorPowerState {
    SENSOR_POWER_STATE_UNDEFINED                    = 0x1,
    SENSOR_POWER_STATE_D0_FULL_POWER                = 0x2,
    SENSOR_POWER_STATE_D1_LOW_POWER                 = 0x3,
    SENSOR_POWER_STATE_D2_STANDBY_POWER_WITH_WAKE   = 0x4,
    SENSOR_POWER_STATE_D3_SLEEP_WITH_WAKE           = 0x5,
    SENSOR_POWER_STATE_D4_POWER_OFF                 = 0x6,
};
